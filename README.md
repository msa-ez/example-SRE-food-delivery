...
## Site Reliability Engineering(SRE) 예제- 음식배달
...
- 산출물 제출
  - 분석/설계 및 구현 산출물 : 1,2일차 분석설계 및 구현단계가 종료되면 도메인에 대한 모델링 결과와 구현소스에 대한 산출물이 제출되어야 함
  - 배포 및 운영 산출물 : 3,4일차 배포 및 운영/모니터링을 위한 CSP 중심의 과정 종료전 관련 산출물이 제출되어야 함 

- 산출물 포멧
  - 단계별 산출물은 해당 README>md 화일을 복사해 사용하거나 프리 포멧으로 제출가능함
  - 제출 시, 반드시 소속과 이름을 기재해야 함
 
- 체크포인트 
  - Domain 주제영역 및 시나리오(반드시 하나의 동기호출 시나리오를 포함해야 함)
  - Domain 분석/설계 모델
  ...
  - 오토스케일 아웃 구현
  - 무정지 재배포 구현
  - Service Mesh기반 오퍼레이션
  - 통합 모니터링
  - 통합 로깅
  - 통합 메시징 플랫폼 설치 및 모니터링
